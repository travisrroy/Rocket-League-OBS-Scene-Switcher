import { Config, GameState, GoalScored, MatchEnded, Slobs } from "./types"
...
class App {
  ...
  private obsClient: Slobs | null; // The OBS websocket client
  ...
  initOBSWebSocket = () => {
    ...
    if (this.OBSAuth === "") {
      ...
      this.obsClient?.login(this.OBSHostname, this.OBSAuth);

      if (this.obsClient?.getConnected()) {
        success.wb("Connected to OBS on " + this.OBSHostname);
  
        const sceneMap = this.obsClient?.getScenes();
        const scenes = [...sceneMap];
        scenes.map((scene) => {
          this.sceneList.push(scene[0]);
        })
      }
      else {
  ...
  updateScene = (sceneName: string, sceneDelay: number) => {
    if(this.sceneList.includes(sceneName)){
      setTimeout(() => { 
        this.obsClient?.setActiveScene(sceneName);
      }, sceneDelay);
    }
    else {
